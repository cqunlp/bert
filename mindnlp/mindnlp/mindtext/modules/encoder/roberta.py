...
class RobertaConfig:
    ...
    def from_dict(cls, yaml_object: dict) -> object:
        """Constructs a `RobertaConfig` from a yaml object of parameters.
        Args:
    ...
    def from_yaml_file(cls, yaml_file: str) -> object:
        """Constructs a `RobertaConfig` from a yaml file of parameters.
        Args:
...
class EmbeddingLookup(nn.Cell):
    """
    A embeddings lookup table with a fixed dictiona ry and size.
    ...
    def __init__(self,
        ...
        self.off_value = Tensor(0.0, mstype.float32)  # Except for the one-hot position
...
class RobertaEncoderCell(nn.Cell):
    ...
    def construct(self, input_tensor: Tensor, attention_mask: Tensor) -> Tensor:
        """
        Args:
            input_tensor: the input for RobertaEncoderCell
            attention_mask: the mask for the attention

        :return:
             output: the output of Roberta encoder
        """
        input_tensor = self.reshape(input_tensor, self.shape)
...
class SecurityCast(nn.Cell):
    ...
    def __init__(self, dst_type: mstype = mstype.float32):
        super(SecurityCast, self).__init__()
        np_type = float
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        return: casted data for preventing the danger
                that the value will overflow or underflow.
        """
        out = self.max_op(x, self.tensor_min_type)
        out = self.min_op(out, self.tensor_max_type)
        return self.cast(out, self.dst_type)
