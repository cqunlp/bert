...
class FewclueEprstmtDataset(CLSBaseDataset):
    ...
    Examples:
        >>> fewclueEprstmt = FewclueEprstmtDataset(tokenizer='spacy', lang='en')
        >>> dataset = fewclueEprstmt()
    """
    ...
    def __init__(self, paths: Optional[Union[str, Dict[str, str]]] = None,
        ...
        super(FewclueEprstmtDataset, self).__init__(sep='\t', name='fewclue eprstmt', **kwargs)
        self._paths = paths
    ...
    def _load(self, path: str) -> DataFrame:
        """
        Load dataset from fewclue_eprstmt file.
        ...
        if "label" in dataset.columns.values:
            dataset = dataset[['id', 'sentence', 'label']]
        else:
            dataset = dataset[['id', 'sentence']]
        dataset.fillna('')
        return dataset
